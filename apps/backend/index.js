...
// ---------- Editor media (authenticated) ----------
const MUSELY_MEDIA_DIR = process.env.MUSELY_MEDIA_DIR || "/data/musely-editor-media";
const IMAGE_BASE_DIR = path.join(MUSELY_MEDIA_DIR, "images");
...
const safeMediaFileId = (raw) => {
  const s = String(raw || "");
  // Only UUID-like filenames from our uploader: hex + "." + ext.
  return /^[a-f0-9]{8,}(\.[a-z0-9]+)?$/i.test(s) ? s : null;
};

const imageStorage = multer.diskStorage({
  destination: (req, _file, cb) => {
    try {
      const userDir = path.join(IMAGE_BASE_DIR, `u${req.user.id}`);
  ...
  filename: (_req, file, cb) => {
    try {
      const ext = imageExtFromMime(file.mimetype) || path.extname(file.originalname) || ".img";
...
function imagePathForUser(userId, fileId) {
  const safe = safeMediaFileId(fileId);
  if (!safe) return null;
  const filePath = path.join(IMAGE_BASE_DIR, `u${userId}`, safe);
  return filePath;
}

app.post("/api/editor/images/upload", requireUser, uploadImage.single("image"), (req, res) => {
  const f = req.file;
  if (!f) return res.status(400).json({ error: "image is required" });
  // `f.filename` is already generated by us (UUID + ext), safe to return as-is.
  res.json({
    id: f.filename,
    url: `/api/editor/images/${encodeURIComponent(f.filename)}`,
    mimeType: f.mimetype,
  });
});

app.get("/api/editor/images/:fileId", requireUser, (req, res) => {
  ...
  res.setHeader("Content-Type", imageMimeFromExt(ext));
