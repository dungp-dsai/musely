...
/**
 * Non-streaming completion against the Musely Agent OpenAI-compatible API.
 * Used for small synchronous tasks (e.g. image caption generation).
 */
export async function completeMuselyAgentChat({ messages, model, target, sessionId }) {
  const { apiKey, baseUrl } = resolveTarget(target);
  if (!apiKey) {
    throw new Error(
      "Musely agent API not configured (set API_SERVER_KEY in ~/.hermes/.env or HERMES_API_SERVER_KEY)"
    );
  }
  ...
  if (!upstream.ok) {
    const text = await upstream.text();
    throw new Error(text || `Hermes chat failed: ${upstream.status}`);
  }
